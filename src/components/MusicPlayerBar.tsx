...
export default function MusicPlayerBar({ sidebarOpen }: { sidebarOpen: boolean }) {
  ...
  const handleAddToPlaylist = () => {
    ...
    // Cegah duplikat
    const alreadyExists = savedPlaylist.some((s: any) => s.id === currentSong.id);

    if (!alreadyExists) {
      const updated = [...savedPlaylist, currentSong];
      localStorage.setItem("playlist", JSON.stringify(updated));
      setAdded(true);

      // Balik ke false setelah 2 detik biar animasi/ikon bisa berubah sementara
    ...
    <div className={playerBaseClass}>
      ...
      <div className="hidden md:flex items-center gap-3 w-1/4 justify-end">
        <button
          ...
          className={`transition ${added ? "text-green-500 scale-110" : "text-zinc-400 hover:text-white"}`}
        >
          <PlusCircle size={18} />
        </button>
